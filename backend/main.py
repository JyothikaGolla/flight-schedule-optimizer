...
def busiest_slots(hour_bucket: int = 1, top: int = 12):
    ...
    return {"labels": agg["hour"].tolist(), "data": agg["count"].tolist()}
...
def top_cascade_flights(window_min: int = 120, top: int = 10):
    ...
    for _, row in df.iterrows():
        ...
        mask = (
            (df["Scheduled_Departure"] > ws)
            & (df["Scheduled_Departure"] <= we)
    ...
    out = df.sort_values("cascade_score", ascending=False).head(top)[
        ["Flight_ID", "Airline", "Scheduled_Departure", "Delay_Minutes", "cascade_score"]
    ]
...
def search_flights(q: str = "", limit: int = 50):
    ...
    if q:
        q = q.lower()
        mask = (
            df["Flight_ID"].str.lower().str.contains(q)
            | df["Airline"].str.lower().str.contains(q)
            | df["Destination"].str.lower().str.contains(q)
        )
...
def optimize_schedule(req: OptimizeRequest):
    ...
    # ---- pick candidate flight ----
    if req.flight_id is None or req.flight_id.strip() == "":
        # If no flight_id provided, select the flight with highest cascade score
        candidate = df.loc[df["cascade_score"].idxmax()]
    else:
        match = df[df["Flight_ID"] == req.flight_id]
        if match.empty:
            return JSONResponse(
                content={"error": f"Flight {req.flight_id} not found"}, status_code=404
            )
        candidate = match.iloc[0]

    orig_time = candidate["Scheduled_Departure"]
    best = {
        "flight": candidate["Flight_ID"],
        "orig_time": str(orig_time),
        "orig_score": int(candidate["cascade_score"]),
        "suggested_shift": 0,
        "new_score": int(candidate["cascade_score"]),
    }

    # ---- test shifts ----
    for shift in range(-req.max_shift, req.max_shift + 1, req.step):
        ...
        mask = (
            (df["Scheduled_Departure"] > ws)
            & (df["Scheduled_Departure"] <= we)
            & (df["Aircraft"] == candidate["Aircraft"])
        )
        downstream = int(mask.sum())
